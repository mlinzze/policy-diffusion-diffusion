...
int main(int argc, char *argv[])
{
	...
	if (argc < 8) {
		printf(" ERROR: Expected arguments: metric model time_period policies scenario parameter_constant parameter_beta_W\n");
		return(1);
	}
	...
	else if (!strcmp(scenario, "counterfactual")) {
		nmontecarlo = 10000;
		treatment = 0;
		ntreated = ncountries;
	}
	...
	int policies_random = 0;
	int n_random_treated = 0;
	char *endptr;
	long policies_integer;

	if (!strcmp(policies, "none")) {
		n_random_treated = 0;
	}
	else if (!strcmp(policies, "current")) {
		n_random_treated = 0;
	}
	else {
		policies_integer = strtol(policies, &endptr, 10);

		if ((endptr == policies) || (*endptr != '\0') || (policies_integer < 0)) {
			printf(" ERROR: policies must be none, current, or a non-negative integer ");
			return(1);
		}

		policies_random = 1;
		n_random_treated = (int) policies_integer;
	}

	if (policies_random == 1) {
		printf("*** Additional random treated: %i \n", n_random_treated);
	}

	if (!strcmp(scenario, "treatment")) {
		if (n_random_treated > ncountries - 1) {
			printf(" ERROR: additional random treated cannot exceed ncountries - 1 in treatment scenario ");
			return(1);
		}
	}
	if (!strcmp(scenario, "counterfactual")) {
		if (n_random_treated > ncountries - 1) {
			printf(" ERROR: random treated cannot exceed ncountries - 1 in counterfactual scenario ");
			return(1);
		}
	}
	...
	char output_directory_temp[512];
	...
	for(unit_treated = 0; unit_treated < ntreated; unit_treated++)
	{
		...
		for (n = 0; n < nmontecarlo; n++)
			...
			if (policies_random == 1) {
				...
				while (n_random_selected < n_random_treated)
				{
					random_unit = rand() % ncountries;

					if (random_unit != unit_treated)
					{
						if (Pn[time_treated][random_unit] == 0.)
						{
							Pn[time_treated][random_unit] = 1.;
							n_random_selected++;
						}
					}
				}
			...
			for (t = time_treated+1; t < ntimes; t++)
				...
				Pn[t][unit_treated] = treatment; // NO endogeneous adoption of treated unit in control simulation
		...
		char unit_name[120];
		snprintf(unit_name, sizeof(unit_name), "%s", countrynames[unit_treated]);
		char string_filename[1024];
		snprintf(string_filename, sizeof(string_filename), "%s/result_%s_%i_%s_%s.csv", output_directory, unit_name, treatment, parameter_constant, parameter_beta_W);
